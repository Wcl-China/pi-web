...
Jiyun Web treats an explicitly empty tool selection as **Chat only**, not merely as
...
themes, or Jiyun base system prompt. Its exact system prompt is the ordered content
of the context files discovered by Jiyun's default loader, including global and
project `AGENTS.md`, `AGENTS.override.md`, and `CLAUDE.md` files. Jiyun Web does not
...
themes, context files, or Jiyun base system prompt. Its exact system prompt is the
...
tools are activated alongside the profile's built-in tools except for Jiyun Web's
...
Jiyun's native session format does not persist the active tool selection. Normal
sessions therefore append versioned `jiyun-web:tool-selection` custom entries:

```json
{
  "type": "custom",
  "customType": "jiyun-web:tool-selection",
  "data": { "version": 1, "tools": [] }
}
```

The latest valid entry is authoritative. No entry means a legacy session and
retains Jiyun's default behavior; an empty `tools` array means Chat only; a nonempty
...
prompt must also be reapplied after Jiyun's `before_agent_start` phase, because the
