...
Jiyun Web's interface uses a small internal i18n layer. It is intentionally kept
...
or Jiyun's own output to a particular language.
...
`localStorage` under `jiyun-locale`.
...
/** Jiyun Web Japanese language package. */
...
the interface harder to map back to commands, configuration fields, or Jiyun
...
Jiyun Web is a user-facing web interface. Internationalization is therefore an
...
support should lower the barrier to using Jiyun Web, not be treated as a measure
...
changing Jiyun's output, or introducing a large translation framework.
