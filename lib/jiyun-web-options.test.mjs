...
const { getHelpText, parseLaunchOptions } = require("../bin/jiyun-web-options.js");
const cliPath = fileURLToPath(new URL("../bin/jiyun-web.js", import.meta.url));
...
test("supports --help and -h without starting the server", () => {
  ...
  assert.match(getHelpText(), /Usage: jiyun-web/);
...
test("CLI writes help and parse errors before exiting", () => {
  ...
  assert.match(help.stdout, /Usage: jiyun-web/);
...
test("supports truthy JIYUN_WEB_NO_OPEN values", () => {
  for (const value of ["1", "true", "TRUE", "yes", "on"]) {
    assert.equal(parseLaunchOptions([], { JIYUN_WEB_NO_OPEN: value }).openBrowser, false);
  }
});

test("does not disable browser opening for false JIYUN_WEB_NO_OPEN values", () => {
  for (const value of ["0", "false", "off", ""]) {
    assert.equal(parseLaunchOptions([], { JIYUN_WEB_NO_OPEN: value }).openBrowser, true);
  }
});
...
test("supports JIYUN_WEB_HOSTNAME without trusting the ambient system HOSTNAME", () => {
  assert.equal(
    parseLaunchOptions([], { HOSTNAME: "container-id" }).hostname,
    "127.0.0.1",
  );
  assert.equal(
    parseLaunchOptions([], { JIYUN_WEB_HOSTNAME: "0.0.0.0" }).hostname,
    "0.0.0.0",
  );
});
