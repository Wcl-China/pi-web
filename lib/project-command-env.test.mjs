...
test("sanitizes host variables using platform casing rules", () => {
  assert.deepEqual(
    sanitizeProjectCommandEnvironment(HOST_ENVIRONMENT, "linux"),
    {
      ...
      JIYUN_USER_SETTING: HOST_ENVIRONMENT.JIYUN_USER_SETTING,
    },
  );
  assert.deepEqual(
    sanitizeProjectCommandEnvironment(
...
test("agent bash removes host variables while preserving SDK and user environment", async () => {
  const original = {
    ...
    JIYUN_USER_SETTING: process.env.JIYUN_USER_SETTING,
  };
  ...
  try {
    ...
    const result = await registeredTool.execute(
      "issue-484",
      {
        command: `node -e 'console.log(JSON.stringify({PORT:process.env.PORT,NODE_ENV:process.env.NODE_ENV,NEXT_RUNTIME:process.env.NEXT_RUNTIME,NEXT_PRIVATE_WORKER:process.env.NEXT_PRIVATE_WORKER,JIYUN_USER_SETTING:process.env.JIYUN_USER_SETTING,JIYUN_SESSION_ID:process.env.JIYUN_SESSION_ID,PATH:process.env.PATH}))'`,
      },
      undefined,
      undefined,
      {
    ...
    assert.equal(childEnvironment.JIYUN_USER_SETTING, "preserved");
    assert.equal(childEnvironment.JIYUN_SESSION_ID, "session-484");
...
test("agent bash reads current shell settings for every execution", async () => {
  let commandPrefix = "export JIYUN_WEB_PREFIX=first";
  const extension = createProjectCommandBashExtension({
    cwd: process.cwd(),
    settings: {
  ...
  const execute = () => registeredTool.execute(
    "settings-reload",
    { command: "printf %s \"$JIYUN_WEB_PREFIX\"" },
    undefined,
    undefined,
    undefined,
  );
  ...
  commandPrefix = "export JIYUN_WEB_PREFIX=second";
...
test("direct bash removes host variables and allows explicit project values", async () => {
  ...
  const operations = createProjectCommandBashOperations({
    ...
    baseEnvironment: {
      ...
      JIYUN_SESSION_ID: "stale-host-value",
    },
  ...
  await operations.exec(
    `NODE_ENV=test PORT=3200 node -e 'console.log(JSON.stringify({PORT:process.env.PORT,NODE_ENV:process.env.NODE_ENV,NEXT_RUNTIME:process.env.NEXT_RUNTIME,JIYUN_USER_SETTING:process.env.JIYUN_USER_SETTING,PATH:process.env.PATH}))'`,
    process.cwd(),
...
test("direct bash updates the platform PATH key", async () => {
  ...
  const cases = [
    ...
    {
      options: { agentBinDir: "C:\\jiyun-agent\\bin", baseEnvironment: { Path: "C:\\Windows" }, platform: "win32" },
      expected: { Path: "C:\\jiyun-agent\\bin;C:\\Windows" },
    },
  ];
...
test("a user extension keeps priority over the Jiyun Web fallback bash tool", async () => {
  const userBash = {
    name: "user-bash",
    factory: (pi) => {
      pi.registerTool({
