...
for (const [rawValue, timeoutMs] of [
  ...
  test(`JIYUN_WEB_IDLE_TIMEOUT_MS=${rawValue} applies to an idle session`, async (t) => {
    const previousValue = process.env.JIYUN_WEB_IDLE_TIMEOUT_MS;
    process.env.JIYUN_WEB_IDLE_TIMEOUT_MS = rawValue;
    try {
      ...
      if (previousValue === undefined) delete process.env.JIYUN_WEB_IDLE_TIMEOUT_MS;
      else process.env.JIYUN_WEB_IDLE_TIMEOUT_MS = previousValue;
    }
  });
}
