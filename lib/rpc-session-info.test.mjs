...
test("lists an accepted new prompt before its session file exists", (t) => {
  ...
  const visible = makeRuntimeSession({
    id: "visible-runtime",
    filePath: join(tmpdir(), "jiyun-web-missing-runtime-session.jsonl"),
    running: true,
    entries: [{
      type: "message",
      id: "u1",
  ...
  const emptyEnsureSession = makeRuntimeSession({
    id: "empty-runtime",
    filePath: join(tmpdir(), "jiyun-web-missing-empty-session.jsonl"),
    running: false,
    entries: [],
  });
...
test("keeps an idle runtime visible once its JSONL file exists", (t) => {
  const previousRegistry = globalThis.__jiyunSessions;
  const dir = mkdtempSync(join(tmpdir(), "jiyun-web-runtime-session-"));
  const filePath = join(dir, "session.jsonl");
  writeFileSync(filePath, "persisted\n");
  globalThis.__jiyunSessions = new Map([["persisted-runtime", makeRuntimeSession({
