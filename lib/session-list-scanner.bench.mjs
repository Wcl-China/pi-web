...
if (process.argv.includes("--persisted-index")) {
  assert.ok(process.env.JIYUN_CODING_AGENT_DIR, "the parent benchmark must supply a temporary agent directory");
...
const root = mkdtempSync(join(tmpdir(), "jiyun-web-scanner-bench-"));
const previousAgentDir = process.env.JIYUN_CODING_AGENT_DIR;
process.env.JIYUN_CODING_AGENT_DIR = root;
const count = 2000;
...
try {
  ...
  for (let trial = 0; trial < 3; trial++) {
    rmSync(join(root, "jiyun-web-session-index.json"), { force: true });
