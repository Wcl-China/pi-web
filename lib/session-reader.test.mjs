...
function resetSessionListState() {
  ...
  globalThis.__jiyunSessionListGeneration = 0;
}

function resetSessionPathState() {
  globalThis.__jiyunSessionPathCache = undefined;
  globalThis.__jiyunPathToSessionIdCache = undefined;
}

function setTestAgentDir(t, agentDir) {
  const previousAgentDir = process.env.JIYUN_CODING_AGENT_DIR;
  process.env.JIYUN_CODING_AGENT_DIR = agentDir;
...
test("reads only a bounded session header, including headers larger than 4 KiB", () => {
  const dir = mkdtempSync(join(tmpdir(), "jiyun-web-header-"));
...
test("returns null for malformed or unbounded session headers", () => {
  const dir = mkdtempSync(join(tmpdir(), "jiyun-web-header-invalid-"));
...
test("session listing reads subagent relations and terminal status without reopening full session files", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "jiyun-web-relation-prefix-"));
  ...
  writeFileSync(filePath, [
    ...
    JSON.stringify({
      type: "custom",
      customType: "jiyun-web:subagent-result",
      id: "result",
      parentId: "meta",
      timestamp: "2026-01-01T00:00:01.000Z",
      data: {
...
test("keeps forward and reverse session path caches in sync", async () => {
  ...
  const filePath = join(tmpdir(), "jiyun-web-cache-test", "..", "cache-test", "session.jsonl");
  ...
  assert.equal(globalThis.__jiyunSessionPathCache?.has(sessionId), false);
  assert.equal(globalThis.__jiyunPathToSessionIdCache?.has(sessionPathKey(filePath)), false);
});

test("resolves a matching session header without a catalogue scan", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "jiyun-web-targeted-session-"));
  ...
  assert.equal(globalThis.__jiyunPathToSessionIdCache?.get(sessionPathKey(filePath)), sessionId);

  resetSessionPathState();
  assert.equal(await resolveSessionIdByPath(filePath), sessionId);
  assert.equal(scans, 0);
  assert.equal(globalThis.__jiyunSessionPathCache?.get(sessionId), filePath);
});

test("does not resolve a parent path outside the default session storage", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "jiyun-web-targeted-outside-"));
  const externalDir = mkdtempSync(join(tmpdir(), "jiyun-web-targeted-external-"));
...
test(
  ...
  async (t) => {
    const agentDir = mkdtempSync(join(tmpdir(), "jiyun-web-targeted-symlink-"));
    const externalDir = mkdtempSync(join(tmpdir(), "jiyun-web-targeted-symlink-external-"));
...
test("falls back to the catalogue when a targeted candidate header is invalid", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "jiyun-web-targeted-fallback-"));
  const sessionId = "target-session";
  const projectDir = join(agentDir, "sessions", "project");
  mkdirSync(projectDir, { recursive: true });
...
test("falls back to the catalogue when a targeted lookup finds duplicate IDs", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "jiyun-web-targeted-duplicate-"));
  const sessionId = "target-session";
