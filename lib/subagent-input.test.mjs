...
test("loads cwd-relative UTF-8 files and formats them as delegated user input", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "jiyun-web-subagent-input-"));
  try {
...
test("rejects files outside cwd, including symlink targets", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "jiyun-web-subagent-input-cwd-"));
  const outside = await mkdtemp(join(tmpdir(), "jiyun-web-subagent-input-outside-"));
  try {
