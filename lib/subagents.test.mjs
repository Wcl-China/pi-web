...
const testAgentDir = await mkdtemp(join(tmpdir(), "jiyun-web-subagents-global-"));
...
test("project profiles override built-ins and round-trip their runtime settings", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "jiyun-web-subagents-"));
  try {
    ...
    const source = await readFile(join(cwd, ".jiyun", "agents", "Explore.md"), "utf8");
...
test("legacy extension selectors are omitted from lightweight profile tools", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "jiyun-web-subagents-"));
  try {
    await mkdir(join(cwd, ".jiyun", "agents"), { recursive: true });
    await writeFile(
      join(cwd, ".jiyun", "agents", "legacy.md"),
...
test("an empty tool selection round-trips without restoring default tools", async () => {
  ...
  try {
    ...
    const source = await readFile(join(cwd, ".jiyun", "agents", `${saved.name}.md`), "utf8");
...
test("project profile directories cannot escape cwd through symbolic links", async (t) => {
  const base = await mkdtemp(join(tmpdir(), "jiyun-web-subagent-boundary-"));
  t.after(() => rm(base, { recursive: true, force: true }));
  const cwd = join(base, "project");
  const outside = join(base, "outside");
  await mkdir(join(cwd, ".agents"), { recursive: true });
  await mkdir(join(cwd, ".jiyun"), { recursive: true });
  await mkdir(outside);
  await writeFile(join(outside, "secret.md"), "---\ndescription: Secret\n---\nprivate\n");

  try {
    await symlink(outside, join(cwd, ".agents", "agents"), process.platform === "win32" ? "junction" : "dir");
    await symlink(outside, join(cwd, ".jiyun", "agents"), process.platform === "win32" ? "junction" : "dir");
  } catch (error) {
    if (error?.code === "EPERM") {
