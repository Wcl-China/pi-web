...
export const JIYUN_WEB_AUTH_USERNAME = "jiyun";
...
export function isWebPasswordEnabled(
  password: string | undefined = process.env.JIYUN_WEB_PASSWORD,
): password is string {
...
export function isValidBasicAuthorization(
  ...
  password = process.env.JIYUN_WEB_PASSWORD,
): boolean {
  ...
  const usernameMatches = secretsEqual(username, JIYUN_WEB_AUTH_USERNAME);
